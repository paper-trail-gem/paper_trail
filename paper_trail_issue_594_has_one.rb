...
class BugTest < ActiveSupport::TestCase
  def test_authorship
    ...
    user.update!({first_name: "Denise", authorship_attributes: {id: user.authorship.id, name: "Steve"}})
    assert_equal 2, user.versions.count

    versioned_user = user.reload.versions.last.reify(has_one: true)
    ...
    assert_equal "Bob", versioned_user.authorship.name

    user.update!({first_name: "Jessica", authorship_attributes: {id: user.authorship.id, name: "Blake"}})
    versioned_user = user.reload.versions.last.reify(has_one: true)
    ...
    assert_equal "Steve", versioned_user.authorship.name
