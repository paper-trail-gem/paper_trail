...
RSpec.describe(PaperTrail, versioning: true) do
  ...
  context "JSON Serializer" do
    ...
    it "reify with JSON serializer" do
      ...
      expect(customer.versions[0].reify).to(be_nil)
      expect(customer.versions[1].reify.name).to(eq("Some text."))
      expect(PaperTrail.config.serializer.load(customer.versions[1].object)).to(eq(original_attributes))
      expect(customer.versions[1].object).to(eq(PaperTrail.config.serializer.dump(original_attributes)))
    end
